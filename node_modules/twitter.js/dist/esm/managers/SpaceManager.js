...
export class SpaceManager extends BaseManager {
    ...
    async fetch(options) {
        ...
        if ('space' in options) {
            ...
            if (!spaceId)
                throw new CustomTypeError('SPACE_RESOLVE_ID');
        ...
        if ('spaces' in options) {
            if (!Array.isArray(options.spaces))
                throw new CustomTypeError('INVALID_TYPE', 'spaces', 'array', true);
    ...
    async fetchByCreatorIds(options) {
        ...
        const fetchedSpaceCollection = new Collection();
        const userIds = options.users.map(user => {
            const userId = this.client.users.resolveId(user);
            if (!userId)
                throw new CustomTypeError('USER_RESOLVE_ID', 'fetch spaces of');
            return userId;
        });
