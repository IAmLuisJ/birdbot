...
import { RequestData, UserBlockResponse, UserFollowResponse, UserMuteResponse, UserUnblockResponse, UserUnfollowResponse, UserUnmuteResponse, User, SimplifiedUser, } from '../structures';
/**
...
export class UserManager extends BaseManager {
    ...
    resolve(userResolvable) {
        ...
        if (user)
            return user;
        if (userResolvable instanceof SimplifiedUser)
    ...
    async fetch(options) {
        ...
        if ('user' in options) {
            ...
            if (!userId)
                throw new CustomError('USER_RESOLVE_ID', 'fetch');
        ...
        if ('users' in options) {
            if (!Array.isArray(options.users))
                throw new CustomTypeError('INVALID_TYPE', 'users', 'array', true);
    ...
    async fetchByUsername(options) {
        ...
        if ('username' in options) {
            ...
            if (typeof username !== 'string')
                throw new CustomTypeError('INVALID_TYPE', 'username', 'string', false);
        ...
        if ('usernames' in options) {
            if (!Array.isArray(options.usernames))
                throw new CustomTypeError('INVALID_TYPE', 'usernames', 'array', true);
    ...
    async follow(targetUser) {
        ...
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'follow');
    ...
    async unfollow(targetUser) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'unfollow');
    ...
    async block(targetUser) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'block');
        const loggedInUserId = this.client.me?.id;
        if (!loggedInUserId)
            throw new CustomError('NO_LOGGED_IN_USER');
        const body = {
    ...
    async unblock(targetUser) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'unblock');
        const loggedInUserId = this.client.me?.id;
        if (!loggedInUserId)
            throw new CustomError('NO_LOGGED_IN_USER');
        const requestData = new RequestData({ isUserContext: true });
    ...
    async mute(targetUser) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'mute');
        const loggedInUserId = this.client.me?.id;
        if (!loggedInUserId)
            throw new CustomError('NO_LOGGED_IN_USER');
        const body = {
    ...
    async unmute(targetUser) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'unmute');
        const loggedInUserId = this.client.me?.id;
        if (!loggedInUserId)
            throw new CustomError('NO_LOGGED_IN_USER');
        const requestData = new RequestData({ isUserContext: true });
    ...
    async fetchFollowers(targetUser, maxResultsPerPage) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'create followers book for');
    ...
    async fetchFollowings(targetUser, maxResultsPerPage) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'create following book for');
    ...
    async fetchLikedTweets(targetUser, maxResultsPerPage) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'create liked book for');
    ...
    async fetchComposedTweets(targetUser, options) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'create tweets book for');
    ...
    async fetchMentions(targetUser, options) {
        const userId = this.resolveId(targetUser);
        if (!userId)
            throw new CustomError('USER_RESOLVE_ID', 'create mentions book for');
        const bookData = { userId };
        if (options?.afterTweet) {
            const afterTweetId = this.client.tweets.resolveId(options.afterTweet);
            if (afterTweetId)
...
async function _UserManager_fetchSingleUser(userId, options) {
    ...
    const fetchedUserCollection = new Collection();
    const queryParameters = this.client.options.queryParameters;
    const query = {
        ids: userIds,
        expansions: queryParameters?.userExpansions,
