...
class TweetManager extends BaseManager_1.BaseManager {
    ...
    async fetch(options) {
        ...
        if ('tweets' in options) {
            if (!Array.isArray(options.tweets))
                throw new errors_1.CustomTypeError('INVALID_TYPE', 'tweets', 'array', true);
    ...
    async like(targetTweet) {
        ...
        return new structures_1.TweetLikeResponse(data);
    }
    /**
     * Unlikes a tweet.
    ...
    async unlike(targetTweet) {
        ...
        const data = await this.client._api
            ...
            .likes(tweetId)
            .delete(requestData);
        return new structures_1.TweetUnlikeResponse(data);
    }
    /**
    ...
    async retweet(targetTweet) {
        ...
        return new structures_1.RetweetResponse(data);
    }
    /**
    ...
    async unRetweet(targetTweet) {
        ...
        const data = await this.client._api
            ...
            .retweets(tweetId)
            .delete(requestData);
        return new structures_1.RemovedRetweetResponse(data);
    }
    /**
    ...
    async fetchRetweetedBy(targetTweet) {
        ...
        const retweetedByUsersCollection = new util_1.Collection();
        if (data.meta.result_count === 0)
            return retweetedByUsersCollection;
        const rawUsers = data.data;
        const rawIncludes = data.includes;
        for (const rawUser of rawUsers) {
            const user = new structures_1.User(this.client, { data: rawUser, includes: rawIncludes });
    ...
    async fetchLikedBy(targetTweet) {
        const tweetId = this.resolveId(targetTweet);
        if (!tweetId)
            throw new errors_1.CustomError('TWEET_RESOLVE_ID', 'fetch liking users');
        const queryParameters = this.client.options.queryParameters;
        const query = {
            expansions: queryParameters?.userExpansions,
            'user.fields': queryParameters?.userFields,
        ...
        const likedByUsersCollection = new util_1.Collection();
        if (data.meta.result_count === 0)
    ...
    async search(query, options) {
        ...
        const searchTweetsBook = new books_1.SearchTweetsBook(this.client, bookData);
    ...
    async count(query, options) {
        ...
        const tweetsCountBook = new books_1.TweetsCountBook(this.client, bookData);
    ...
    async create(options) {
        const data = new structures_1.TweetPayload(this.client, options).resolveData();
        const requestData = new structures_1.RequestData({ body: data, isUserContext: true });
...
async function _TweetManager_fetchSingleTweet(tweetId, options) {
    ...
    const fetchedTweetCollection = new util_1.Collection();
    const queryParameters = this.client.options.queryParameters;
    const query = {
        ids: tweetIds,
        expansions: queryParameters?.tweetExpansions,
    ...
    const tweetId = this.resolveId(targetTweet);
    if (!tweetId)
        throw new errors_1.CustomError('TWEET_RESOLVE_ID', `${isHidden ? 'hide' : 'unhide'}`);
    const body = {
        hidden: isHidden,
    };
    const requestData = new structures_1.RequestData({ body, isUserContext: true });
    const data = await this.client._api.tweets(tweetId).hidden.put(requestData);
    return new structures_1.TweetReplyHideUnhideResponse(data);
};
